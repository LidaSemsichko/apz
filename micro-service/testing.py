...
async def post_one_with_retry(client: httpx.AsyncClient, user_id: str, amount: int, txid: str) -> None:
    while True:
        try:
            r = await client.post(
        ...
        except (httpx.ReadTimeout, httpx.ConnectTimeout, httpx.PoolTimeout, httpx.ConnectError):
            await asyncio.sleep(0.01)
...
async def scenario_1_distinct_users() -> None:
    ...
    tasks = [asyncio.create_task(worker(i, users[i], REQS_PER_CLIENT, 1)) for i in range(CLIENTS)]
...
async def scenario_2_same_user() -> None:
    ...
    tasks = [asyncio.create_task(worker(i, target_user, REQS_PER_CLIENT, 1)) for i in range(CLIENTS)]
